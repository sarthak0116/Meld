...
function HudInput({ id, label, type = "text", value, onChange, placeholder, error, autoComplete, suffix }) {
  return (
    <div>
      ...
      {error && (
        <p className="mt-1.5 font-['Share_Tech_Mono'] text-[11px] text-red-500/80">
          // {error}
        </p>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-6 right-4 sm:right-[60px] z-20 font-['Orbitron'] text-[10px] font-black tracking-[3px] text-[#0b0c0b]/25 uppercase">
        SECURE LOGIN <span className="text-[#e53e3e]">+</span>
      </div>
        ...
        <div className="relative bg-[#cbd4cc] border border-[#0b0c0b]/15 px-8 py-10">
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
            ...
            <div className="flex justify-end">
              <span className="font-['Rajdhani'] text-[11px] font-bold tracking-[1px] text-[#0b0c0b]/30 uppercase">
                Forgot? — coming soon
              </span>
            </div>

            {/* Submit — solid dark HUD button */}
            <button
              type="submit"
              disabled={isLoading}
              className="relative w-full bg-[#0b0c0b] py-3.5 font-['Orbitron'] text-[12px] font-black tracking-[3px] text-[#cbd4cc] uppercase transition-all duration-200 hover:bg-[#e53e3e] hover:text-[#0b0c0b] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              style={{ borderRadius: 0 }}
            >
          ...
          <div className="my-6 flex items-center gap-3">
            <div className="h-px flex-1 bg-[#0b0c0b]/10" />
            <span className="font-['Rajdhani'] text-[10px] font-bold tracking-[2px] text-[#0b0c0b]/30 uppercase">or</span>
            <div className="h-px flex-1 bg-[#0b0c0b]/10" />
          </div>
          ...
          {socialToast && (
            <p className="mt-2 text-center font-['Share_Tech_Mono'] text-[10px] text-[#0b0c0b]/40">// {socialToast}</p>
          )}
          ...
          <p className="mt-5 text-center font-['Share_Tech_Mono'] text-[11px] text-[#0b0c0b]/40">
            No account?{" "}
            <Link to="/signup" className="text-[#0b0c0b] font-bold underline hover:text-[#e53e3e] transition-colors" style={{ textDecorationColor: "currentColor" }}>
              CREATE ONE
            </Link>
          </p>
        </div>
